...
class InstallerEngine:
    ...
    def install(self, setup):
        ...
        try:
            ...
            if os.path.exists(
                    "/target/var/lib/AccountsService/users/%s" %
                    live_user):
                self.do_run_in_chroot(
                    "rm -f /var/lib/AccountsService/users/%s" %
                    live_user)
            ...
            self.update_progress(
                total=our_total,
                current=our_current,
                message=_("Initializing the new installation"))
            ...
            self.update_progress(
                total=our_total,
                current=our_current,
                message=_("Removing live configuration (installer)"))
            ...
            self.update_progress(
                total=our_total,
                current=our_current,
                message=_("Adding users to system"))
            ...
            self.update_progress(
                total=our_total,
                current=our_current,
                message=_("Setting keyboard options"))
            keyboarddir = "/etc/X11/xorg.conf.d"
            self.do_run_in_chroot("mkdir -p %s" % keyboarddir)
            keyboardfh = open(
                "/target/etc/X11/xorg.conf.d/00-keyboard.conf", "w")
...
EndSection\n""" % (setup.keyboard_model, setup.keyboard_layout))
    ...
    def do_goofi(self, our_total, our_current):
        ...
        entfile = os.path.join(
            self.get_loader_dir("/target/boot/efi"),
            "loader.conf")
